...
**Tech Stack:** TypeScript, Node.js (≥20), `@anthropic-ai/sdk`, `openai`, `commander`, `zod`, `zod-to-json-schema`, `ink` + `react` + `ink-text-input` (terminal UI), `vitest` + `ink-testing-library`, `tsup`.
...
npm install @anthropic-ai/sdk openai commander zod zod-to-json-schema ink react ink-text-input
npm install -D typescript vitest tsup @types/node @types/react ink-testing-library
```

Version note: ink 5+, `ink-text-input` 6+, and react 18 are ESM — they match this project's `"type": "module"` setup.
...
The `"jsx": "react-jsx"` (automatic runtime) lets `.tsx` files use JSX without importing React explicitly.
...
(The `.tsx` glob lets the ink `<App>` test in Task 13 run. esbuild handles JSX via the tsconfig `jsx` setting.)
...
### Task 12: System prompt + UI store

**Files:**
- Create: `src/system-prompt.ts`, `src/ui/store.ts`
- Test: `test/system-prompt.test.ts`, `test/ui/store.test.ts`

**Interfaces:**
- Consumes: `PromptFn` from `src/permissions/index.ts` (the store's `requestPrompt` matches it).
- Produces:
  - `buildSystemPrompt(cwd: string): string` — includes the `mdd` identity, the cwd, the OS
    platform, and a short guideline block (read before editing; explain before mutating).
  - `src/ui/store.ts`:
    - `type TranscriptItem = { kind: 'user'; text: string } | { kind: 'assistant'; text: string } | { kind: 'tool'; name: string; input: unknown }`
    - `interface UiState { transcript: TranscriptItem[]; streaming: string; status: 'idle' | 'busy'; pendingPrompt: string | null }`
    - `class UiStore` with **arrow-bound** methods (safe to pass as callbacks):
      `getState()`, `subscribe(fn)`, `addUser(text)`, `appendStreaming(delta)`,
      `commitStreaming()`, `addTool(name, input)`, `setStatus(s)`,
      `requestPrompt(message): Promise<string>`, `resolvePrompt(answer): void`.
    - `appendStreaming` matches `onText`; `addTool` matches `onToolStart`; `requestPrompt`
      matches `PromptFn` — these are the three seams the agent loop and permission gate use.
...
Run: `npx vitest run test/system-prompt.test.ts test/ui/store.test.ts`
Expected: FAIL — modules not found.
...
```bash
git add -A
git commit -m "feat: system prompt and observable UI store"
```

---

### Task 13: Ink app + mount helper

**Files:**
- Create: `src/ui/app.tsx`, `src/ui/index.tsx`
- Test: `test/ui/app.test.tsx`

**Interfaces:**
- Consumes: `UiStore`, `TranscriptItem` from `src/ui/store.ts`; `ink`, `ink-text-input`, `react`.
- Produces:
  - `src/ui/app.tsx`: `function App({ store, onSubmit }: { store: UiStore; onSubmit: (line: string) => void }): JSX.Element` — subscribes to the store via `useSyncExternalStore`, renders committed transcript in `<Static>`, the live streaming text, a "…thinking" line when busy, and a `<TextInput>` when idle or a permission prompt is pending. On submit: if a prompt is pending → `store.resolvePrompt(value)`; else → `onSubmit(value)`.
  - `src/ui/index.tsx`: `mountApp(store: UiStore, onSubmit: (line: string) => void): { unmount(): void; waitUntilExit(): Promise<void> }` — renders `<App>` via ink's `render`. Re-exports `UiStore` and `TranscriptItem`.
...
Note: `useSyncExternalStore(store.subscribe, store.getState, store.getState)` — `getState`
returns the same object reference until `set()` replaces it, so this does not loop; ink
re-renders only when the store actually changes.

- [ ] **Step 4: Write `src/ui/index.tsx`**
...
export function mountApp(store: UiStore, onSubmit: (line: string) => void): { unmount(): void; waitUntilExit(): Promise<void> } {
  ...
  return { unmount: instance.unmount, waitUntilExit: instance.waitUntilExit };
}
```
...
```bash
git add -A
git commit -m "feat: ink app component and mount helper"
```

---

### Task 14: CLI entry — auth, one-shot, REPL (ink-wired)
...
**Interfaces:**
- Consumes: everything — `loadConfig`/`saveConfig`, `getProvider`, `buildRegistry`, `createGate`, `runTurn`, `buildSystemPrompt`, `UiStore`/`mountApp` from `src/ui/index.tsx`.
- Produces: the executable entry. Commands/flags:
  - `mdd auth login` — prompts for provider choice and API key(s), saves via `saveConfig` (uses `node:readline/promises` directly, since ink isn't mounted yet).
  - `mdd [prompt...]` — one-shot if a prompt is given (mounts ink, runs one turn, unmounts); REPL if not (mounts ink, loops on input).
...
Note: ink handles Ctrl-C by default (unmounts and resolves `waitUntilExit`), so the REPL
exits cleanly without a manual SIGINT handler.
...
```
Expected: help text listing the `auth` command, the `[prompt...]` argument, and
`--provider/--model/--yes` options. No crash.
...
Run: `npm test`
Expected: PASS — all unit + provider + loop + UI-store + ink-app tests green; the smoke
suite is skipped (no `MDD_E2E`).

- [ ] **Step 5: Manual verification (real key)**

Run:
```bash
node dist/cli.js auth login   # enter a real Anthropic key
node dist/cli.js "list the files in this directory"   # one-shot; ink renders the turn
node dist/cli.js                                        # REPL; type a request, Ctrl-C to exit
```
Expected: streamed answer in the ink UI; a `list_dir`/`run_shell` tool call runs without
prompting (read-only); a write shows an inline `[y/n/a]` permission prompt.

- [ ] **Step 6: Commit**

```bash
git add -A
git commit -m "feat: mdd CLI entry with auth, one-shot, and ink REPL"
```

---

### Task 15: README + package polish
...
- Default anthropic/`claude-opus-4-8`, switchable → Task 10 factory + Task 14 `resolveSetup`. ✓
- Six v1 tools (read/write/edit/list/shell/git) → Tasks 3–6. ✓
- Confirm-on-mutate + `--yes` + denylist → Tasks 5, 7, 14. ✓
- Per-user config, `0600`, env overrides → Task 1. ✓
- Ink terminal UI (Static transcript, streaming, inline prompts) → Tasks 12–13. ✓
- REPL + one-shot → Task 14. ✓
- Bounded loop + size-capped results → Tasks 2 (truncate), 11 (MAX_ROUNDS). ✓
- Errors-as-values → every tool handler + loop denial path. ✓
- Testing: unit + provider translation + fake-provider loop + UI-store + ink-app + opt-in E2E → Tasks 3–14. ✓
- Distribution/README → Task 15. ✓
- GitLab/Odoo explicitly deferred → not in plan (correct; v2). ✓

**Placeholder scan:** No TBD/TODO; every code step contains complete code. ✓

**Type consistency:** `Message`/`ContentBlock`/`ToolUseBlock`/`ToolResultBlock` defined in Task 2, used unchanged in Tasks 8–14. `ProviderEvent`/`StreamOptions`/`LLMProvider` defined in Task 8, implemented identically in Tasks 8–10, consumed in Task 11. `Tool`/`ToolResult`/`ToolContext`/`ToolSchema` from Task 2 used in Tasks 3–7, 11. `PromptFn`/`PermissionGate`/`createGate` from Task 7; the store's `requestPrompt` matches `PromptFn` and is passed to `createGate` in Task 14. `UiStore` methods `appendStreaming`/`addTool`/`requestPrompt` (Task 12) match the loop's `onText`/`onToolStart` and the gate's `prompt` (Tasks 11, 14). `Config`/`loadConfig`/`saveConfig` from Task 1 used in Tasks 10, 14. Names verified consistent across tasks. ✓

**Ink-specific checks:** `UiStore` uses arrow-bound methods so `store.appendStreaming` etc.
survive being passed as bare callbacks. `useSyncExternalStore` reads `getState` (stable
reference between `set()` calls) — no render loop. `.tsx` files are covered by the Task 1
`vitest.config.ts` glob and the `jsx: react-jsx` tsconfig setting. ✓
